  ...
  try {
    ...
    switch (type) {
      case FETCH_BY_UUID:
        ...
        break;
      case EDIT:
        await client(token).put(action.spending.$id.url.toString(), action.updates, {
          'If-Match': action.spending.$version
        })
        dispatch(editSuccess(Spending.fromJSON({
          ...action.spending.toJSON(),
          ...{
            ...action.updates,
            bookedAt: action.updates.bookedAt ? action.updates.bookedAt.toISOString() : undefined
          },
